...
CITY_DATA=readtable('CITY.xlsx');
CITY_time=CITY_DATA{:,1};
CITY_velocity=CITY_DATA{:,2};
%to export data to simulink
velocity=[CITY_time,CITY_velocity];
stop_time=CITY_time;
...
MDL_motormodel_maxtorque_out=500;%Nm
MDL_motormodel_max_power_out=100000;%W
...
grid on
figure(2)
plot(t,actual_velocity,CITY_time,CITY_velocity);
xlabel('time')
ylabel('velocity_mph')
...
% Reference Input Value (Drive cycle velocity)
   ref_value = CITY_velocity; 

% Find Overshoot (OS)
%interpolate the actual velocity to size actual velocity  
actual_velocity_resampled=interp1(t,actual_velocity,CITY_time);
...
t_max=CITY_time(index_of_peak_value);
