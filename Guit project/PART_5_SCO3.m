...
SCO3_velocity=SCO3_DATA{:,2};
%to export data to simulink
velocity=[SCO3_time,SCO3_velocity];
stop_time=SCO3_time;
...
out=sim('PART_5_COMBINED_SCO3_MODEL');
t=out.time;
...
grid on
figure(2)
plot(t,actual_velocity,SCO3_time,SCO3_velocity);
xlabel('time')
ylabel('velocity_mph')
...
% Reference Input Value (Drive cycle velocity)
   ref_value = SCO3_velocity; 

% Find Overshoot (OS)
%interpolate the actual velocity to size actual velocity  
actual_velocity_resampled=interp1(t,actual_velocity,SCO3_time);
...
t_max=SCO3_time(index_of_peak_value);
