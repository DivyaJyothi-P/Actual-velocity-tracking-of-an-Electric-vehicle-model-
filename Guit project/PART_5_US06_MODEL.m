...
US06_velocity=US06_DATA{:,2};%mphCLC
%to export data to simulink
velocity=[US06_time,US06_velocity];
stop_time=US06_time;
...
grid on
figure(2)
plot(t,actual_velocity,US06_time,US06_velocity);
xlabel('time')
ylabel('velocity_mph')
...
% Reference Input Value (Drive cycle velocity)
   ref_value = US06_velocity; 

% Find Overshoot (OS)
%interpolate the actual velocity to size actual velocity from [6011 1] to
%[601 1]
actual_velocity_resampled=interp1(t,actual_velocity,US06_time);
...
t_max=US06_time(index_of_peak_value);
